...
struct ResourcesView: View {
    ...
    var body: some View {
        KumoPage(title: "Resources") {
            Form {
                Section(String(localized: "Geo Data")) {
                    ...
                    Toggle(String(localized: "GeoIP DAT Mode"), isOn: geoDatModeBinding)
                    Toggle(String(localized: "Auto Update"), isOn: geoAutoUpdateBinding)
                    TextField("Update Interval (hours)", value: geoUpdateIntervalBinding, format: .number)
                    Button(String(localized: "Update Geo Data")) {
                        Task { await store.upgradeGeoData() }
                    }
                    .disabled(store.status.state != .running || store.isLoading)
                }

                Section(String(localized: "Proxy Providers")) {
                    if store.proxyProviders.isEmpty {
                        ...
                        Button(String(localized: "Update All Providers")) {
                ...
                Section(String(localized: "Rule Providers")) {
